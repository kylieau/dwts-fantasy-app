...
- **Test changes before finishing.** Before considering a task done: run the dev server or build (`npm run build`) and confirm it succeeds, run lint, run `npm test` (Vitest — currently just the scoring engine) if the change touches pure logic, and exercise the actual feature path (not just "it compiles") when the change is user-facing.
